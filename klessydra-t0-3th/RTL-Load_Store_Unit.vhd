...
architecture LSU of Load_Store_Unit is
  ...
  signal flush_hart_int_wire        : std_logic_vector(harc_range); --inutilizzato
  signal flush_hart_int             : std_logic_vector(harc_range); --inutilizzato
...
--  ╚══════╝╚══════╝ ╚═════╝     ╚══════╝   ╚═╝   ╚═╝  ╚═══╝ ╚═════╝  --
------------------------------------------------------------------------
  debug_lsu_nen: if debug_en = 0 generate
  ...
  end generate debug_lsu_nen;

  debug_lsu_en: if debug_en = 1 generate
    LSU_sync : process(clk_i, rst_ni)
    begin
      ...
      if rst_ni = '0' then
       ...
       LS_WB <= (others => '0');
        ...
        busy_LS_lat <= busy_LS_int or core_busy_LS_int;
        LS_WB_EN_int    <= LS_WB_EN_wire_int;
        if (ls_instr_req = '0' and busy_LS_lat = '0') or halt_req(harc_EXEC) = '1' or halt_req_wire(harc_EXEC) = '1' then
          --LS_WB_EN_int <= '0';
        elsif LS_instr_req = '1' or busy_LS_lat = '1' then
          --LS_WB_EN_int <= '0';
          case state_LS_int is  
    ...
    LSU_comb : process(
                        ...
                        RS2_Data_IE, decoded_instruction_LS, amo_store_lat, data_rvalid_i,
                        data_addr_internal_int, RS2_Data_IE_lat, store_err, data_addr_internal_lat,
                        halt_req, halt_req_wire, harc_EXEC
                      ) --VHDL1993
